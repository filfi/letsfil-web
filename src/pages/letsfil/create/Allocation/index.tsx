  ...
  return (
    <>
      ...
      <Form form={form} layout="vertical" initialValues={data ?? {}} onFinish={handleSubmit}>
        <Form.Item label="募集商权益" name="raiserShare" rules={[{ required: true, message: '请输入募集商权益' }]}>
          <Input type="number" max={100} min={0} suffix="%" placeholder="请输入百分比" />
        </Form.Item>

        <Form.Item label="投资者权益" name="investorShare" rules={[{ required: true, message: '请输入投资者权益' }]}>
          <Input type="number" max={100} min={0} suffix="%" placeholder="请输入百分比" />
        </Form.Item>

        <Form.Item label="服务商权益" name="servicerShare" rules={[{ required: true, message: '请输入服务商权益' }]}>
          <Input type="number" max={100} min={0} suffix="%" placeholder="请输入百分比" />
        </Form.Item>
