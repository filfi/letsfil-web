...
export type ImTokenConnectorOptions = {
  /**
...
export class ImTokenConnector extends InjectedConnector {
  ...
  #UNSTABLE_shimOnConnectSelectAccount: ImTokenConnectorOptions['UNSTABLE_shimOnConnectSelectAccount'];
      ...
      if (this.#UNSTABLE_shimOnConnectSelectAccount && this.options?.shimDisconnect && !this.storage?.getItem(this.shimDisconnectKey)) {
        ...
        if (isConnected)
          ...
          try {
            ...
            // Not all ImToken injected providers support `wallet_requestPermissions` (e.g. ImToken iOS).
            // Only bubble up error if user rejects request
            if (this.isUserRejectedRequestError(error)) throw new UserRejectedRequestError(error as Error);
            // Or ImToken is already open
