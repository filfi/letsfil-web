...
export default function useRaiseRate(data?: API.Plan | null) {
  ...
  // 建设者权益
  const investRate = useMemo(() => toFixed(U.accMul(priorityRate, U.accDiv(U.accSub(100, opsRatio), 100)), 2), [priorityRate, opsRatio]);
  // 保证金权益
  const opsRate = useMemo(() => toFixed(U.accMul(priorityRate, U.accDiv(opsRatio, 100)), 2), [priorityRate, opsRatio]);
  // 服务商权益
  const servicerRate = useMemo(() => U.accSub(data?.op_server_share ?? 5, 0), [data?.op_server_share]);
  // filfi 协议权益
  const ffiRate = useMemo(() => toFixed(U.accMul(inferiorityRate, 0.08), 2, 2), [inferiorityRate]);
  // 主办人权益
