...
import { Avatar as AntAvatar, Image, Skeleton } from 'antd';
import useBreakpoint from 'antd/es/grid/hooks/useBreakpoint';
import Jazzicon, { jsNumberForAddress } from 'react-jazzicon';
import { responsiveArray } from 'antd/es/_util/responsiveObserver';
import type { AvatarProps as AntAvatarProps, Breakpoint } from 'antd';
...
function matchSize(bk: Breakpoint, sizes: Partial<Record<Breakpoint, number>>) {
  let size = sizes['xs'];
  const bks: Breakpoint[] = ['xs', 'sm', 'md', 'lg', 'xl', 'xxl'];

  for (const _bk of bks) {
    if (sizes[_bk]) {
      size = sizes[_bk];
    }
...
const Avatar: React.FC<AvatarProps> = ({ className, address, size, src, ...props }) => {
  ...
  const diameter = useMemo(() => {
    ...
    if (typeof customSize === 'object') {
      ...
      return matchSize(currentBreakpoint, customSize);
    }
    ...
    if (isUrl(src)) {
      return (
        <Image
          ...
          height="100%"
          preview={false}
          placeholder={<Skeleton.Avatar active size={diameter} />}
  ...
  return <AntAvatar className={className} size={diameter} {...props} src={renderSrc()} />;
};
