...
const EditPane: React.FC<EditPaneProps> = ({ onCancel, onConfirm }) => {
  ...
  const balance = useMemo(() => {
    if (borrow && collateral) {
      return Math.max(accSub(collateral.amount, borrow.amount), 0);
    }
            ...
            rules={[
              {
                ...
                message: formatMessage({
                  id: 'pages.borrow.form.label.amount',
                }),
              },
            ]}
          >
            <Input
              type="number"
              placeholder="0"
              addonAfter={<span className={styles.unit}>FIL</span>}
            />
          </Form.Item>

          <p className="d-flex align-items-center justify-content-between my-4 text-gray">
            <span>
              <FormattedMessage id="pages.borrow.form.label.balance" />{' '}
              {formatAmount(balance)} FIL
            </span>
            <a
              className="ms-2 text-gray-dark fs-bold"
              href="#"
              onClick={handleAll}
            >
