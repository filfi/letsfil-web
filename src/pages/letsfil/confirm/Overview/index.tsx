...
export default function ConfirmOverview() {
  ...
  const disabled = useMemo(
    () => planState !== RaiseState.WaitSeverSign,
    [planState],
  );
  ...
  const { loading: submitting, run: handleSubmit } = useLoadingify(async () => {
    ...
    if (
      ...
      data.service_provider_address?.toLowerCase()
    ) {
      Modal.alert({
        icon: 'warn',
        title: '非指定服务商地址',
      ...
      <Skeleton active loading={loading} paragraph={{ rows: 10 }}>
        {isSigned ? (
          <div className="text-center">
            ...
            <h4 className="fs-2 fw-bold mb-2">确认成功</h4>
            <p className="text-gray">该募集计划将显示在市场列表中</p>
          </div>
        ) : (
          <>
                ...
                <div className={classNames('d-flex mb-3', styles.code)}>
                  ...
                  <p
                    className={classNames(
                      'flex-fill mx-1 mb-0 fw-bold',
                      styles.command,
                    )}
                  >
                    lotus-miner actor propose-change-beneficiary --really-do-it
                    &lt;contractAddress&gt; &lt;quota&gt; &lt;expiration&gt;
                    <br />
                    quota: 10000000
                    <br />
